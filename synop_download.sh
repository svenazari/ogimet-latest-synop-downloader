...
[[ ! -d /home/$(whoami)/saved_synop/$folim ]] && mkdir /home/$(whoami)/saved_synop/$folim
[[ ! -d /home/$(whoami)/saved_synop/$folim/$folim2 ]] && mkdir /home/$(whoami)/saved_synop/$folim/$folim2

FILE=/home/$(whoami)/scripts/synop.txt #file which will be copied 
dattim=$(date -u +"%Y%m%d%H") #file name after copying
if test -f "$FILE"; then #check if file which needs to be copied exist - if does, copy it
  cp $FILE /home/$(whoami)/saved_synop/$folim/$folim2/synop_hr_$dattim.txt
  rm $FILE
else
  exit
fi
